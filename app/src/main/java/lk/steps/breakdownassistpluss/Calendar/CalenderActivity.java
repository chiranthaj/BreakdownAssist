...
public abstract class CalenderActivity extends AppCompatActivity implements WeekView.EventClickListener, MonthLoader.MonthChangeListener, WeekView.EventLongPressListener, WeekView.EmptyViewLongPressListener {
    ...
    protected void onResume() {
        super.onResume();
        registerReceiver(broadcastReceiver, new IntentFilter("lk.steps.breakdownassistpluss.CalenderActivityBroadcastReceiver"));
    }

    List<Interruption> interruptions = null;

    BroadcastReceiver broadcastReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            Log.d("TEST", "CalenderActivityBroadcastReceiver");
            String _interruptions = intent.getStringExtra("interruptions");
            Type type = new TypeToken<List<Interruption>>(){}.getType();
            interruptions = new Gson().fromJson(_interruptions, type);
            // Log.e("TEST", "Number-" + interruptions.size());
           // Log.e("TEST", "Number-" + interruptions.get(0).Description);
           // Log.e("TEST", "Number-" + interruptions.get(0).StartTime);
            //Log.d("TEST", "="+interruptions);
